...
inline double computeSecondDerivative(const std::vector<double>& u,
                         ...
                         int rightIndex,
                         int recvIndex) {
    double leftValue = (index == 0) ? recvLeft[recvIndex] : u[leftIndex];
    double rightValue = (index == upperBoundary) ? recvRight[recvIndex] : u[rightIndex];

    return (leftValue - 2.0 * u[centerIndex] + rightValue) / hSquared;
...
int main(int argc, char** argv) {
    ...
    MPI_Comm comm;
    int gridDimensions[dimentionsCount] = {0, 0, 0};
    int periodicDirections[dimentionsCount] = {0, 1, 1};
    int gridCoordinates[dimentionsCount];
